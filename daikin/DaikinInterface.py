...
class DaikinInterface(Appliance):
    ...
    async def _update_settings_get_control(self):
    ...
    async def _update_settings_get_sensor(self):
        """Update settings to set on Daikin device."""
        # start with current values
        current_val = await self._get_resource('aircon/get_sensor_info')
    ...
    async def get_control(self):
        await self._update_settings_get_control()

    async def get_sensor(self):
        await self._update_settings_get_sensor()
